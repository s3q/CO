...
const print = (...obj) => { return console.log(...obj) }
const print_err = (...obj) => { return console.error(...obj) }

jquery = false

try {
    // check if jquery is found or not 
    $()
    jquery = true
...
if (jquery) {
    (() => { // slide :
        ...
        element.each(function (index, ele) {
            ...
            if (ele.getAttributeNode('data-slide')) {
                ...
                if (dependent) {
                    ...
                    // This condition is useful in avoiding some errors :
                    if (ele.getAttribute('data-duration')) {
                    ...
                    if (dependent.attr('aria-expanded') === 'true') {
                        ...
                        // This condition is useful in avoiding some errors :
                        if (ele.getAttributeNode('data-screen')) {
    ...
    (() => { // fade :
        ...
        element.each(function (index, ele) {
            ...
            if (ele.getAttributeNode('data-fade')) {
                ...
                if (dependent) {
                    ...
                    // Make fade alert :
                    function make_fade_alert(el, dep, dur, data_ev) {
                        ...
                        if (data_ev === 'click' || data_ev === 'onclick') {
                            ...
                            el.addEventListener('click', function () {
                                ...
                                // fade alert :
                                if (dep.children('.alert-window').css('margin-top') != '0px') {

                                    dep.fadeToggle(dur / 1);

                                    dep.children('.alert-window').animate({
                                        marginTop: '0',
                                        opacity: '1'
                                    });
                                    ...
                                    dep.fadeToggle(dur / .5);

                                }

                            });

                            dep.children('.alert-window').click(function (event) {
                    ...
                    function make_fades(el, dep, dur, data_ev) {
                        ...
                        } else if (el.getAttributeNode('data-fade').value === 'alert') {

                            // fade alert :
                            make_fade_alert(el, dep, dur, data_ev);
    ...
    (() => { // hide & show :
        ...
        element.each(function (index, ele) {
            ...
            if (ele.getAttributeNode('data-target')) {
                ...
                dependent.each(function (index, dep) {
                    ...
                    if (ele.getAttributeNode('data-pattern')) {
...
(() => { // events :
    ...
    let ele_prevent_default = document.querySelectorAll('[prevent-default]');

    // Select all elements:
    ele_prevent_default.forEach(function (ele) {

        // Screening process :
        // Elements enter this function that has (attr (prevent-default)) only
        if (ele.getAttributeNode('prevent-default')) {
    ...
    let ele_focus = document.querySelectorAll('[focus]');

    ele_focus.forEach(function (ele) {

        // Elements enter this function that has (attr (focus)) only
        if (ele.getAttributeNode('focus')) {
    ...
    let ele_blur = document.querySelectorAll('[blur]');

    ele_blur.forEach(function (ele) {

        // Elements enter this function that has (attr (blur)) only
        if (ele.getAttributeNode('blur')) {
    ...
    let ele_click = document.querySelectorAll('[click]');

    ele_click.forEach(function (ele) {

        // Elements enter this function that has (attr (click)) only
        if (ele.getAttributeNode('click')) {
...
(() => {
    ...
    // Defining a variable (dependent) and its usefulness Defining the element related to the event to the previous one such as (click, dblclick, change ..) :
    let dependent = document.querySelectorAll('[data-target]');

    // Select all elements:
    dependent.forEach(function (ele_dependent) {

        let attr_role = ele_dependent.getAttributeNode("role").value

        if (attr_role === "switch") {

            let ele_togglebox = document.querySelector(`#${ele_dependent.getAttributeNode("data-target").value}`);

            let eledisplay_box = ele_togglebox.getAttributeNode("data-display") ? ele_togglebox.getAttributeNode("data-display").value : "block"

            Array.from(ele_dependent.children).forEach((elebutton_toggle, i) => {

                elebutton_toggle.setAttribute("data-index", i)

                elebutton_toggle.addEventListener("click", function () {

                    Array.from(ele_togglebox.children).forEach(function (elebox) {

                        if (elebutton_toggle.getAttributeNode("data-index").value === elebox.getAttributeNode("data-index").value) {
                            elebox.style.display = eledisplay_box;
                        } else {
                ...
                Array.from(ele_togglebox.children).forEach((elebox, i) => {

                    elebox.setAttribute("data-index", i)
        ...
        } else if (attr_role === "search") {


            ele_dependent.children.forEach(function (ele_dep) {

                // Element was extracted with id specified in (attr(data-target))
                let ele_search = document.querySelectorAll(`#${ele_dep.getAttributeNode('data-target').value}`);

                // Select all elements:
                ele_search.forEach(function (ele_se) {

                    // The dependent must contain(attr(data-description)) ... in order to perform the search
                    if (ele_dep.getAttributeNode('data-description')) {

                        // Stores the display value before the change 
                        let DP = ele_dep.style.display;

                        // input > onkeyup 
                        ele_search.addEventListener("keyup", function () {
                    ...
                    window.addEventListener('load', function () {
                        ...
                        automatic_navigation.forEach(function (ele_automatic_navigation) {
                            ...
                            if (ele_automatic_navigation.getAttributeNode('data-automatic-navigation').value == 'true') {
                                ...
                                }, ele_automatic_navigation.getAttributeNode('data-timeout') ? ele_automatic_navigation.getAttributeNode('data-timeout').value : 10000);
